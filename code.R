...
graficos <- function(name, width, height, pos, pos_ipc){
  #pos es la del tipo de cambio, pos_ipc la de la inflacion.
  ...
  plot3 <- plot.fevd.boot(eval(as.name(paste(deparse(substitute(name)), ".FEVD.boot", sep=""))), m, H, pos_ipc)
  print(plot3)
  dev.off()
...
plot.sirf.boot(SVAR_2.SIRF.boot, m, H, 1)
dev.off()
...
plot.sirf.boot(SVAR.SIRF.c.boot, m, H, 1)
dev.off()
...
plot.fevd.boot(SVAR_2.FEVD.boot, m, H, 2)
dev.off()
# 
# # ERPT (bootstrap)
...
# Punto 3 #### 
#Parte A: CON precios internacionales.
...
SVAR_3_s1_c_pcom <- SVAR(VAR, Amat = Amat, Bmat = Bmat, lrtest = FALSE)
...
SVARS(SVAR_3_s1_c_pcom, 3, 2)
graficos(SVAR_3_s1_c_pcom, 800, 800, 2, 3)
# 
# # IRF (bootstrap)
...
SVAR_3_s2_c_pcom <- SVAR(VAR, Amat = Amat, Bmat = Bmat, lrtest = FALSE)
...
SVARS(SVAR_3_s2_c_pcom, 3, 2)
graficos(SVAR_3_s2_c_pcom, 800, 800, 2, 3)
...
SVAR_3_s3_c_pcom <- SVAR(VAR, Amat = Amat, Bmat = Bmat, lrtest = FALSE)
...
SVARS(SVAR_3_s3_c_pcom, 3, 2)
graficos(SVAR_3_s3_c_pcom, 800, 800, 2, 3)
# 
# # IRF (bootstrap)
# SVAR.SIRF.boot <- SVAR.sirf.boot(SVAR, Amat, Bmat, Yb, pmax, H, a, R)
# 
...
#Punto 3 Parte B: SIN precios internacionales.
...
SVAR_3_s1_s_pcom <- SVAR(VAR, Amat = Amat, Bmat = Bmat, lrtest = FALSE)
...
SVARS_s_pcom(SVAR_3_s1_s_pcom, 2, 1)
graficos(SVAR_3_s1_s_pcom, 800, 800, 1, 2)
...
SVAR_3_s2_s_pcom <- SVAR(VAR, Amat = Amat, Bmat = Bmat, lrtest = FALSE)
...
SVARS_s_pcom(SVAR_3_s2_s_pcom, 2, 1)
graficos(SVAR_3_s2_s_pcom, 800, 800, 1, 2)
...
SVAR_3_s3_s_pcom <- SVAR(VAR, Amat = Amat, Bmat = Bmat, lrtest = FALSE)
...
SVARS_s_pcom(SVAR_3_s3_s_pcom, 2, 1)
graficos(SVAR_3_s3_s_pcom, 800, 800, 1, 2)
...
Yd_5_1 <- cbind(pcom, Yd, var_wages_real) # Raw data in log
...
graficos(SVAR_5_s_pcom, 800, 800, 1, 2)
# 
# # IRF
...
graficos(SVAR_5_c_pcom, 800, 800, 2, 3)
# 
# # IRF
...
Yb <- boot.rb.replicate(VAR, Yd0,  pmax, R)
SVARS_s_pcom(SVAR_6_s_pcom, 2, 1)
graficos(SVAR_6_s_pcom, 800, 800, 1, 2)
...
graficos(SVAR_6_c_pcom, 800, 800, 2, 3)
...
SVAR_7_c_pcom<- SVAR(VAR, Amat = Amat, Bmat = Bmat, lrtest = FALSE)
...
SVARS(SVAR_7_c_pcom, 3, 2)
graficos(SVAR_7_c_pcom, 800, 800, 2, 3)

#Parte B: sin precios internacionales
...
SVAR_7_s_pcom <- SVAR(VAR, Amat = Amat, Bmat = Bmat, lrtest = FALSE)

H <- 18 # Horizon IR
H_ERPT <- 120 # Horizon ERPT - variance decomp.
a <- 0.95 # Confidence level
R <- 500 # No. of bootstrap replications
Yb_7_s_pcom  <- boot.rb.replicate(VAR, Yd0, pmax, R)
Yb <- Yb_7_s_pcom 

SVARS_s_pcom(SVAR_7_s_pcom, 2, 1)
graficos(SVAR_7_s_pcom, 800, 800, 1, 2)
...
SVAR_8_c_pcom <- SVAR(VAR, Amat = Amat, Bmat = Bmat, lrtest = FALSE)
...
R <- 500 # No. of bootstrap replications
Yb <- boot.rb.replicate(VAR, Yd0, pmax, R)
SVARS(SVAR_8_c_pcom, 3, 2)
graficos(SVAR_8_c_pcom, 800, 800, 2, 3)
...
SVAR_8_s_pcom <- SVAR(VAR, Amat = Amat, Bmat = Bmat, lrtest = FALSE)
...
H_ERPT <- 120 # Horizon ERPT - variance decomp.
a <- 0.95 # Confidence level
R <- 200 # No. of bootstrap replications
Yb <- boot.rb.replicate(VAR, Yd0, pmax, R)

SVARS_s_pcom(SVAR_8_s_pcom, 2, 1)
graficos(SVAR_8_s_pcom, 800, 800, 1, 2)
...
SVAR_9_c_pcom <- SVAR(VAR, Amat = Amat, Bmat = Bmat, lrtest = FALSE)
...
H_ERPT <- 120 # Horizon ERPT - variance decomp.
a <- 0.95 # Confidence level
R <- 100 # No. of bootstrap replications
Yb <- boot.rb.replicate(VAR, Yd0, pmax, R)

SVARS(SVAR_9_c_pcom, 3, 2)
graficos(SVAR_9_c_pcom, 800, 800, 2, 3)
...
SVAR_9_s_pcom <- SVAR(VAR, Amat = Amat, Bmat = Bmat, lrtest = FALSE)
...
SVARS_s_pcom(SVAR_9_s_pcom, 2, 1)
graficos(SVAR_9_s_pcom, 800, 800, 1, 2)
